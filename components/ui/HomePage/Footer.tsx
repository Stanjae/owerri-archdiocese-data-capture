...
const Footer = () => (
    <footer className="pt-32 sm:pt-44">
        <div className="custom-screen text-gray-600 dark:text-foreground">
            <div className="flex flex-wrap gap-y-10 items-center justify-between">
                <div className="space-y-4">
                    <Link href="/" className="inline-block">
                        <Brand />
                    </Link>
                    <h2 className="text-gray-800 dark:text-foreground/80 text-lg font-semibold sm:text-2xl">Owarch-Data Capture tool</h2>
                    <p className="max-w-md">
                        We help companies to get the best strategies to improve their data collection.
                    </p>
            ...
            <div className="mt-10 py-10 border-t flex-row-reverse items-center justify-between sm:flex">
                <ul className="flex flex-wrap items-center gap-4 sm:text-sm">
                    {
                        navigation.map((item, idx) => (
                            <li key={idx} className="font-medium text-gray-700 dark:text-foreground/70 hover:text-gray-900 duration-150">
                                <Link href={item.href}>
                                    {item.name}
                                </Link>
                            </li>
                ...
                <p className="mt-6 sm:mt-0">© {year} Owarch DataCapture Inc. All rights reserved.</p>
            </div>
        </div>
    </footer>
)
